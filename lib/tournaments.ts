...
type TournamentRow = { slug: string; name: string; sport: string; category: string; event_date: string; place: string; country: TournamentCountry; region: string | null; surface: string | null; registered_count: number; capacity: number; fee: number | string; organizer_name: string; playing_fields: number; match_duration: number; start_time?: string; turnover_minutes?: number; schedule_slot_minutes?: number; has_lunch_break?: boolean; lunch_break_start?: string | null; lunch_break_duration?: number | null; game_system?: GameSystem; format_settings?: { mode?: string; sizes?: number[]; finalMode?: FinalMode } | null; qualifiers_per_group?: number | null; third_place_match?: boolean; tournament_invited_teams?: { team_name: string }[] };
export type TournamentRepeatSource = {
  ...
  region: string | null;
  surface: string | null;
  formatSettings: { mode: "custom"; sizes: number[]; finalMode: FinalMode } | null;
...
function mapTournament(row: TournamentRow): Tournament {
  ...
  return { slug: row.slug, name: row.name, sport: row.sport, category: row.category, date: row.event_date,
    displayDate: date.format(new Date(`${row.event_date}T12:00:00Z`)), city: row.place, country: row.country, region: row.region, surface: row.surface,
    ...
    rules: [`Počet hracích plôch: ${row.playing_fields}`, `Dĺžka zápasu: ${row.match_duration} minút`, `Systém hry: ${row.format_settings?.mode === "custom" ? `Vlastné skupiny (${row.format_settings.sizes?.join(" + ") ?? ""})` : gameSystemLabel(row.game_system)}`, ...(row.start_time ? [`Začiatok zápasov: ${row.start_time.slice(0, 5)}`, `Časový blok: ${row.schedule_slot_minutes} minút (prestávka ${row.turnover_minutes} minút)`] : []), ...(row.has_lunch_break ? [`Obedná prestávka: ${row.lunch_break_start?.slice(0, 5)}, ${row.lunch_break_duration} minút`] : [])],
    participants: row.tournament_invited_teams?.map((team) => team.team_name) ?? [] };
}

export async function getPublicTournaments() {
  const supabase = await createClient();
  const { data } = await supabase.from("tournaments").select("slug,name,sport,category,event_date,place,country,region,surface,format_settings,registered_count,capacity,fee,organizer_name,playing_fields,match_duration,start_time,turnover_minutes,schedule_slot_minutes,has_lunch_break,lunch_break_start,lunch_break_duration,game_system,qualifiers_per_group,third_place_match").order("event_date");
...
export async function getTournamentBySlug(slug: string) {
  ...
  const supabase = await createClient();
  const { data } = await supabase.from("tournaments").select("slug,name,sport,category,event_date,place,country,region,surface,format_settings,registered_count,capacity,fee,organizer_name,playing_fields,match_duration,start_time,turnover_minutes,schedule_slot_minutes,has_lunch_break,lunch_break_start,lunch_break_duration,game_system,qualifiers_per_group,third_place_match,tournament_invited_teams(team_name)").eq("slug", slug).maybeSingle();
...
export async function getTournamentRepeatSource(slug: string): Promise<TournamentRepeatSource | null> {
  ...
  const { data } = await supabase
    .from("tournaments")
    .select("slug,organizer_id,name,sport,category,place,country,region,surface,format_settings,preferred_finish,capacity,playing_fields,match_duration,start_time,has_lunch_break,lunch_break_start,lunch_break_duration,game_system,qualifiers_per_group,third_place_match,fee,tournament_type,team_visibility,tournament_invited_teams(club_id,team_name)")
    .eq("slug", slug)
  ...
  return {
    ...
    place: data.place,
    country: data.country as TournamentCountry,
    region: data.region,
    surface: data.surface,
    formatSettings: data.format_settings as TournamentRepeatSource["formatSettings"],
    preferredFinish: data.preferred_finish ? String(data.preferred_finish).slice(0, 5) : null,
...
export async function getTournamentEditSource(slug: string): Promise<TournamentEditSource | null> {
  ...
  const userId = auth?.claims?.sub;
  if (!userId) return null;
  const { data } = await supabase.from("tournaments").select("id,slug,organizer_id,name,sport,category,event_date,place,country,region,surface,format_settings,preferred_finish,capacity,playing_fields,match_duration,start_time,has_lunch_break,lunch_break_start,lunch_break_duration,game_system,qualifiers_per_group,third_place_match,fee,tournament_type,team_visibility,tournament_invited_teams(club_id,team_name)").eq("slug", slug).eq("organizer_id", userId).maybeSingle();
  if (!data) return null;
  ...
  const tournamentTypes = { public: "Verejný", invitation: "Pozvánkový", combined: "Kombinovaný" } as const;
  const teamVisibilities = { all: "Zobrazovať všetkým", accepted: "Iba prijatým tímom", hidden: "Nezobrazovať" } as const;
  return { slug: data.slug, name: data.name, sport: data.sport, category: data.category, eventDate: data.event_date, place: data.place, country: data.country as TournamentCountry, region: data.region, surface: data.surface, formatSettings: data.format_settings as TournamentRepeatSource["formatSettings"], preferredFinish: data.preferred_finish ? String(data.preferred_finish).slice(0, 5) : null,
