...
export function TournamentCard({ tournament }: { tournament: Tournament }) {
  ...
  return (
    <article className="group flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-card transition duration-300 hover:-translate-y-1 hover:border-arena-100 hover:shadow-xl">
      ...
      <div className="mt-6 space-y-3 text-sm text-slate-600"><p className="detail-row"><CalendarIcon />{tournament.displayDate}</p><p className="detail-row"><PinIcon />{tournament.city}{tournament.region ? ` · ${tournament.region}` : ""}, {tournament.country}</p>{tournament.surface && <p className="text-sm font-semibold text-arena-700">Povrch: {tournament.surface}</p>}<p className="detail-row"><UsersIcon />{tournament.registered} z {tournament.capacity} {tournament.participantLabel}</p></div>
