...
export default async function AdminTeamsPage({ searchParams }: { searchParams: Promise<{ stav?: string }> }) {
  ...
  return <section className="mx-auto max-w-5xl px-5 py-12 lg:px-8 lg:py-16">
    <div className="flex items-end justify-between"><div><p className="eyebrow">Administrácia</p><div className="mt-2 flex items-center gap-3"><h1 className="text-3xl font-extrabold">Žiadosti o tímy</h1>{rows.length + proposedTeams.length > 0 && <span className="rounded-full bg-amber-500 px-3 py-1 text-sm font-extrabold text-white">{rows.length + proposedTeams.length} otvorených</span>}</div><p className="mt-2 text-slate-600">Schválením potvrdíte, že používateľ môže zastupovať konkrétny tím.</p></div><div className="flex gap-3"><Link className="btn-secondary" href="/admin/timy/prehlad">Registrované tímy</Link><Link className="btn-secondary" href="/admin/pouzivatelia">Používatelia</Link></div></div>
    {params.stav === "approved" && <p className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 font-semibold text-emerald-800">Žiadosť bola úspešne schválená.</p>}
    {params.stav === "rejected" && <p className="mt-6 rounded-2xl border border-slate-300 bg-slate-50 px-5 py-4 font-semibold text-slate-700">Žiadosť bola zamietnutá.</p>}
    {params.stav === "chyba" && <p className="mt-6 rounded-2xl border border-red-300 bg-red-50 px-5 py-4 font-semibold text-red-800">Žiadosť sa nepodarilo spracovať. Nebola vykonaná žiadna zmena.</p>}
    ...
    <h2 className="mt-12 flex items-center gap-3 text-2xl font-extrabold">Nové kluby a tímy {proposedTeams.length > 0 && <span className="rounded-full bg-amber-100 px-3 py-1 text-sm text-amber-800">{proposedTeams.length} neschválených</span>}</h2>
