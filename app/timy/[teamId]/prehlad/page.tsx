...
export default async function TeamDashboardPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  const [{ data, error }, { data: unread }] = await Promise.all([
    ...
    supabase.rpc("get_team_chat_unread_count", { p_team_id: teamId }),
  ]);
  ...
  return <section className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
    ...
    <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <DashboardCard href={`/timy/${teamId}/chat`} icon={<UsersIcon />} title="Chat" text="Správy tímu a dôležité oznamy" badge={Number(unread ?? 0)} />
...
function DashboardCard({ href, icon, title, text, badge }: { href: string; icon: React.ReactNode; title: string; text: string; badge?: number }) {
  return <Link className="group relative rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-arena-300 hover:shadow-md" href={href}><span className="grid h-11 w-11 place-items-center rounded-2xl bg-arena-50 text-arena-700">{icon}</span>{badge !== undefined && badge > 0 && <span className="absolute right-5 top-5 rounded-full bg-amber-100 px-2.5 py-1 text-xs font-extrabold text-amber-900">{badge} nových</span>}<h2 className="mt-5 text-lg font-extrabold text-ink">{title}</h2><p className="mt-1 text-sm leading-6 text-slate-600">{text}</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-extrabold text-arena-700">Otvoriť <ArrowIcon className="h-4 w-4 transition group-hover:translate-x-1" /></span></Link>;
}
