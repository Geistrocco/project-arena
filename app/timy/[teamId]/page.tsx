...
export default async function TeamRosterPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  return <section className="mx-auto max-w-3xl px-5 py-12 sm:py-16">
    ...
    <div className="mt-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><p className="eyebrow">Tímová súpiska</p><h1 className="mt-2 text-3xl font-extrabold text-ink">{team.name}</h1><p className="mt-2 text-slate-600">{team.category} · sezóna {team.season} · {players.filter((player) => player.status === "active").length} aktívnych hráčov</p>
